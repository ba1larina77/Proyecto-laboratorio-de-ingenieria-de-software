      ...
      {(tab === "register" || tab === "edit") && (
        <div className="rounded-2xl p-6" style={{ background: "#fff", boxShadow: "0 3px 16px rgba(74,55,40,0.08)" }}>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-5">
            ...
            {/* Editorial — bloqueado en edición o si autocompletado CON valor.
                Si Google Books no trajo editorial, queda editable para escribirla a mano. */}
            <div>
              <label className="block text-xs font-medium mb-1.5" style={{ color: "#4A3728", display: "flex", alignItems: "center", gap: 6 }}>
                Editorial *
                {bibLocked && <LockedBadge />}
                {!bibLocked && gbFound && form.publisher.trim() !== "" && (
                  <span className="text-[10px] px-1.5 py-0.5 rounded font-semibold flex items-center gap-1" style={{ background: "rgba(96,108,56,0.12)", color: "#606C38" }}>
                    🔒 Auto
                  </span>
                )}
                {!bibLocked && gbFound && form.publisher.trim() === "" && (
                  <span className="text-[10px] px-1.5 py-0.5 rounded font-semibold flex items-center gap-1" style={{ background: "rgba(212,163,115,0.18)", color: "#8B6914" }}>
              ...
              <input name="publisher" type="text"
                ...
                readOnly={bibLocked || (editId === null && gbFound && form.publisher.trim() !== "")}
                tabIndex={bibLocked || (editId === null && gbFound && form.publisher.trim() !== "") ? -1 : undefined}
                className="w-full px-4 py-2.5 rounded-xl text-sm outline-none"
                style={{
                  ...inputStyle("publisher"),
                  ...((bibLocked || (editId === null && gbFound && form.publisher.trim() !== "")) ? lockedInputStyle : {})
                }} />

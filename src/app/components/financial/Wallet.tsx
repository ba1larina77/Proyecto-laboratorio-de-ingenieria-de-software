...
const MAX_BALANCE = 1_000_000; // Tope máximo de saldo permitido (COP)
...
export function Wallet() {
  ...
  const CARDS_KEY = `biblion_cards_${user?.id ?? "guest"}`;

  const [tab, setTab]                   = useState<"overview" | "cards" | "history">("overview");
  const [cards, setCards]               = useState<Card[]>(() => {
    try {
      const raw = localStorage.getItem(`biblion_cards_${user?.id ?? "guest"}`);
      if (raw) return JSON.parse(raw) as Card[];
    } catch { /* ignore */ }
    // Solo mostrar tarjetas demo para el usuario demo predeterminado
    return user?.id === "U-CLI-001B" ? INITIAL_CARDS : [];
  });
  ...
  // Persistir tarjetas por usuario en localStorage
  useEffect(() => {
  ...
  // Estado para errores de recarga
  ...
  function handleRecharge() {
    ...
    if (!amount || amount < 5000) {
      setRechargeError("El monto mínimo de recarga es $5.000");
      return;
    }
    if (amount > MAX_BALANCE) {
      setRechargeError(`El monto máximo por recarga es ${fmt(MAX_BALANCE)}`);
      return;
    }
    if (balance + amount > MAX_BALANCE) {
      const disponible = MAX_BALANCE - balance;
      setRechargeError(
        disponible <= 0
          ? "Tu billetera ya alcanzó el saldo máximo permitido."
          : `Con esta recarga superarías el límite de ${fmt(MAX_BALANCE)}. Puedes recargar máximo ${fmt(disponible)}.`
      );
      return;
    }
    if (!cvv || cvv.length < 3) {
      setRechargeError("El CVV es obligatorio (3 dígitos)");
      return;
    }
      ...
      {tab === "overview" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="rounded-2xl p-6" style={{ background: "#fff", boxShadow: "0 3px 16px rgba(74,55,40,0.08)" }}>
            <h3 className="font-bold text-base mb-1" style={{ fontFamily: "'Playfair Display', serif", color: "#4A3728" }}>
              Recargar saldo
            </h3>
            <p className="text-xs mb-4" style={{ color: "#6B5344" }}>
              Saldo máximo permitido: <strong>{fmt(MAX_BALANCE)}</strong>
              {balance > 0 && ` · Disponible para recargar: ${fmt(Math.max(0, MAX_BALANCE - balance))}`}
            </p>
            ...
            <button onClick={handleRecharge}
              ...
              {recharging
                ...
                : `Recargar ${rechargeAmount ? fmt(rechargeAmount) : customAmount ? fmt(Number(customAmount.replace(/\D/g, ""))) : ""}`}
            </button>
            {rechargeError && (
              <p className="text-xs mt-2 flex items-center gap-1.5 font-medium" style={{ color: "#C0392B" }}>

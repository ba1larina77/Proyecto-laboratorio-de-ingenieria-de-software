...
function getSuggestions(query: string, books: Book[]) {
  ...
  books.forEach(b => (b.categories ?? []).forEach(c => allCats.add(c)));
  Array.from(allCats).filter(c => c.toLowerCase().includes(q)).slice(0, 2)
...
export function Catalog() {
  ...
  // Construir lista de categorías dinámicamente desde el inventario
  // Incluye todas las categorías presentes en los libros, incluyendo "Histórico Agotado"
  ...
  // CU-CR-001: filtro por título, autor, ISBN + categoría + condición
  const filtered = books.filter(b => {
    const matchCat = category === "Todos" || (b.categories ?? []).includes(category);
    const q = debouncedSearch.toLowerCase();
    const matchSearch = !q
      || b.title.toLowerCase().includes(q)
      || b.author.toLowerCase().includes(q)
      || b.isbn.toLowerCase().includes(q);
    const matchCondition = conditionFilter === "todos"
      || (conditionFilter === "nuevo" && (b.condition === "nuevo" || b.condition === undefined && b.isNew))
      || (conditionFilter === "usado" && b.condition === "usado");
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-2 mb-4">
        <span className="text-xs font-semibold" style={{ color: "#6B5344" }}>Condición:</span>
        {(["todos", "nuevo", "usado"] as const).map(opt => (
          <button key={opt} onClick={() => setConditionFilter(opt)}
            className="px-3 py-1.5 rounded-full text-xs font-semibold border transition-all capitalize"
            style={conditionFilter === opt
              ? { background: "#606C38", borderColor: "#606C38", color: "#fff" }
              : { background: "#fff", borderColor: "#E8C99A", color: "#6B5344" }}>
            {opt === "todos" ? "Todos" : opt === "nuevo" ? "📗 Nuevo" : "📙 Usado"}
          </button>
        ))}
      </div>
      ...
      {(() => {
        // Safeguard: inventario vacío
        if (books.length === 0) {
          return (
            <div className="rounded-2xl p-14 text-center" style={{ background: "#fff", boxShadow: "0 3px 16px rgba(74,55,40,0.08)" }}>
              <p className="text-5xl mb-4">📦</p>
              <p className="font-bold text-xl mb-2" style={{ fontFamily: "'Playfair Display', serif", color: "#4A3728" }}>
                Catálogo vacío
              </p>
              <p className="text-sm" style={{ color: "#6B5344" }}>
                Aún no hay libros registrados en el sistema. El administrador puede agregar libros desde el panel de gestión.
              </p>
            </div>
          );
        }
        ...
        const userPrefs = (user?.temasPreferencia ?? []).map(p => p.toLowerCase());
        ...
        // ── DASHBOARD PRINCIPAL: siempre muestra TODOS los libros ──
        // La sección de preferencias aparece al final como complemento (no como filtro)
        return (
          <div className="space-y-10">

            {/* Sección Novedades para suscritos — libros de los últimos 7 días que coincidan con preferencias */}
            {isSubscribed && (() => {
              const SEVEN_DAYS_MS = 7 * 24 * 60 * 60 * 1000;
              const newBooks = sorted.filter(b => {
                if (!b.addedDate) return false;
                if (Date.now() - new Date(b.addedDate).getTime() > SEVEN_DAYS_MS) return false;
                if (userPrefs.length === 0) return true;
                return (b.categories ?? []).some(cat =>
                  userPrefs.some(p => cat.toLowerCase().includes(p) || p.includes(cat.toLowerCase()))
                );
              });
              return (
                <section>
                  <div className="flex flex-wrap items-baseline gap-3 mb-4 pb-2 border-b" style={{ borderColor: "#E8C99A" }}>
                    <h2 className="text-2xl font-bold" style={{ fontFamily: "'Playfair Display', serif", color: "#4A3728" }}>
                      ⭐ Novedades para ti
                    </h2>
                    <span className="text-xs px-2 py-0.5 rounded-full font-semibold" style={{ background: "#606C38", color: "#fff" }}>
                      Últimos 7 días
                    </span>
                  </div>
                  {newBooks.length > 0
                    ? renderBooks(newBooks)
                    : <p className="text-sm py-4 italic" style={{ color: "#6B5344" }}>
                        No hay novedades recientes que coincidan con tus preferencias.
                      </p>}
                </section>
              );
            })()}

            {/* Todos los libros — siempre visible independientemente de preferencias */}
            <section>
              <div className="flex items-baseline gap-3 mb-4 pb-2 border-b" style={{ borderColor: "#E8C99A" }}>
                <h2 className="text-2xl font-bold" style={{ fontFamily: "'Playfair Display', serif", color: "#4A3728" }}>
                  📚 {isSubscribed ? "Catálogo completo" : "Todos los libros"}
                </h2>
              </div>
              {renderBooks(sorted)}
            </section>

            {/* ── SECCIÓN DE PREFERENCIAS al final — solo para suscritos con prefs ── */}
            {isSubscribed && userPrefs.length > 0 && (() => {
              const preferenceBooks = sorted.filter(b =>
                (b.categories ?? []).some(cat =>
                  userPrefs.some(pref =>
                    cat.toLowerCase().includes(pref) || pref.includes(cat.toLowerCase())
                  )
                )
              );
              return (
                <section>
                  <div className="flex flex-wrap items-baseline gap-3 mb-4 pb-2 border-b" style={{ borderColor: "#E8C99A" }}>
                    ...
                    <div className="flex flex-wrap gap-1.5">
                      {(user?.temasPreferencia ?? []).map(pref => (
                        <span key={pref}
                          className="text-xs px-2 py-0.5 rounded-full capitalize"
                          style={{ background: "rgba(74,55,40,0.08)", color: "#4A3728", fontWeight: 600 }}>
                          {pref}
                        </span>
                      ))}
                    </div>
                  </div>
                  {preferenceBooks.length > 0 ? (
                    renderBooks(preferenceBooks)
                  ) : (
                    <div className="rounded-2xl p-10 text-center" style={{ background: "#fff", boxShadow: "0 3px 16px rgba(74,55,40,0.08)" }}>
                      <p className="text-3xl mb-3">📖</p>
                      <p className="font-bold mb-1" style={{ fontFamily: "'Playfair Display', serif", color: "#4A3728" }}>
                        Sin libros para tus preferencias
                      </p>
                      <p className="text-sm" style={{ color: "#6B5344" }}>
                        Aún no tenemos libros en las categorías que elegiste. ¡Pronto habrá novedades!
                      </p>
                    </div>
                  )}
                </section>
      ...
      {selectedBook && (
        <div className="fixed inset-0 z-[500] flex items-center justify-center p-4"
          ...
          <div className="w-full max-w-lg rounded-2xl overflow-hidden bg-white relative"
            style={{ maxHeight: "90vh", overflowY: "auto", boxShadow: "0 24px 80px rgba(74,55,40,0.3)" }}
            onClick={e => e.stopPropagation()}>
            
            <button 
              onClick={() => setSelectedBook(null)}
              className="absolute top-4 right-4 w-8 h-8 rounded-full flex items-center justify-center bg-white/80 backdrop-blur-sm border shadow-sm hover:bg-white transition-colors z-10"
              style={{ color: "#4A3728", borderColor: "rgba(0,0,0,0.1)" }}>
              <X className="w-4 h-4" />
            </button>

            <div className="flex gap-5 p-6">
              <img 
                src={selectedBook.cover.replace("w=300", "w=600").replace("h=450", "h=800")} 
                alt={selectedBook.title}
                loading="eager"
                decoding="async"
                className="w-32 h-44 rounded-xl object-cover flex-shrink-0 shadow-md"
                onError={e => { (e.target as HTMLImageElement).src = "https://images.unsplash.com/photo-1543002588-bfa74002ed7e?w=600&q=80"; }} />
              <div className="flex-1">
                ...
                <p className="text-sm mb-2" style={{ color: "#6B5344" }}>{selectedBook.author}</p>
                  <div className="flex flex-wrap gap-1 mb-3">
                    {(selectedBook.categories ?? []).map(cat => (
                      <span key={cat} className="text-xs px-2 py-0.5 rounded-full" style={{ background: "#F5EDD3", color: "#4A3728" }}>
                        {cat}
                      </span>
                    ))}
                    {selectedBook.isNew && (
                ...
                {/* Módulo de Realidad Aumentada (M-RA-HU1): ver el libro en 3D / AR.
                    Disponible para cualquier usuario; abre la ruta /ar/:bookId. */}
                <button
                  onClick={() => { const id = selectedBook.id; setSelectedBook(null); navigate(`/ar/${id}`); }}
                  className="w-full mt-2 py-2.5 rounded-xl text-sm font-semibold hover:opacity-90 flex items-center justify-center gap-2"
                  style={{ background: "#0A0E14", color: "#00E5FF", border: "1px solid #00E5FF55" }}>
  ...
  return (
    <div className="rounded-2xl overflow-hidden transition-all hover:shadow-lg cursor-pointer group"
      ...
      <div className="relative h-52 overflow-hidden bg-gray-100">
        <img 
          src={book.cover.includes("unsplash") ? `${book.cover}&w=400&q=75` : book.cover} 
          alt={book.title}
          loading="lazy"
          decoding="async"
          className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
          onError={e => { (e.target as HTMLImageElement).src = "https://images.unsplash.com/photo-1543002588-bfa74002ed7e?w=400&q=60"; }} />
        {book.condition === "usado" && (
          <span className="absolute top-2 right-2 text-xs font-bold px-2 py-0.5 rounded-full"
            style={{ background: "#8B6914", color: "#fff" }}>Usado</span>
        )}
        {book.isNew && book.condition !== "usado" && (
  ...
  return (
    <div className="flex gap-4 p-4 rounded-2xl transition-all hover:shadow-md cursor-pointer"
      ...
      <div className="w-14 h-20 rounded-xl overflow-hidden flex-shrink-0 bg-gray-100">
        <img 
          src={book.cover.includes("unsplash") ? `${book.cover}&w=150&q=60` : book.cover} 
          alt={book.title} 
          loading="lazy"
          decoding="async"
          className="w-full h-full object-cover"
          onError={e => { (e.target as HTMLImageElement).src = "https://images.unsplash.com/photo-1543002588-bfa74002ed7e?w=150&q=50"; }} />
      </div>

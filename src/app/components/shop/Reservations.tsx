...
import { useShop, fmt, MAX_RESERVATIONS } from "../../store/ShopContext";
...
function CountdownTimer({ res, onExpire }: { res: Reservation; onExpire: (id: string) => void }) {
  ...
  if (diff <= 0) return <span className="text-xs font-bold px-2 py-1 rounded-lg" style={{ background: "rgba(192,57,43,0.1)", color: "#C0392B" }}>Expirada</span>;
  const h = Math.floor(diff / 3600000);
  const m = Math.floor((diff % 3600000) / 60000);
  const s = Math.floor((diff % 60000) / 1000);
  const isSafe = diff > 10 * 3600000;
  return (
    <span className="text-xs font-bold px-2.5 py-1 rounded-lg" style={{ background: isSafe ? "rgba(96,108,56,0.1)" : "rgba(192,57,43,0.1)", color: isSafe ? "#606C38" : "#C0392B" }}>
  ...
  return (
    <div>
      <div className="flex flex-wrap gap-4 justify-between items-start mb-7">
        <div>
          <h1 className="text-3xl font-bold mb-1" style={{ fontFamily: "'Playfair Display', serif", color: "#4A3728" }}>Mis reservas</h1>
          <p className="text-sm" style={{ color: "#6B5344", opacity: 0.8 }}>Máximo {MAX_RESERVATIONS} reservas activas · Se liberan tras 24 horas</p>
        </div>
        <div className="flex gap-6 rounded-xl px-5 py-3.5" style={{ background: "#fff", boxShadow: "0 2px 12px rgba(74,55,40,0.10)" }}>
          <div className="text-center">
            <p className="text-xs mb-1" style={{ color: "#6B5344" }}>Activas</p>
            <p className="text-2xl font-bold" style={{ fontFamily: "'Playfair Display', serif", color: "#4A3728" }}>{active.length}</p>
          </div>
          <div className="w-px" style={{ background: "#EDE0C4" }} />
          <div className="text-center">
            <p className="text-xs mb-1" style={{ color: "#6B5344" }}>Límite</p>
            <p className="text-2xl font-bold" style={{ fontFamily: "'Playfair Display', serif", color: "#6B5344" }}>{MAX_RESERVATIONS}</p>
          </div>
        </div>
      </div>
      ...
      {active.length === 0 ? (
        <div className="rounded-2xl p-12 text-center" style={{ background: "#fff", boxShadow: "0 3px 16px rgba(74,55,40,0.08)" }}>
          ...
          <p className="text-xs mb-4" style={{ color: "#6B5344", opacity: 0.7 }}>Explora el catálogo para reservar libros</p>
        </div>
      ) : (
        <div className="space-y-4">
          {active.map(res => (
            <div key={res.id} className="flex gap-5 rounded-2xl p-5 transition-all hover:shadow-md" style={{ background: "#fff", boxShadow: "0 3px 16px rgba(74,55,40,0.08)", borderLeft: "4px solid #D4A373" }}>
              ...
              <div className="flex-1 min-w-0">
                <h3 className="font-bold text-base mb-0.5 leading-tight" style={{ fontFamily: "'Playfair Display', serif", color: "#4A3728" }}>{res.book.title}</h3>
                ...
                <div className="flex flex-wrap gap-3 items-center mb-3">
                  <span className="text-xs flex items-center gap-1" style={{ color: "#6B5344" }}>
                    ...
                    {res.createdAt.toLocaleDateString("es-CO", { day: "numeric", month: "short", hour: "2-digit", minute: "2-digit" })}
                  </span>
                  <span className="text-xs font-semibold px-2 py-0.5 rounded-full" style={{ background: "rgba(212,163,115,0.2)", color: "#4A3728" }}>{res.book.category}</span>
                </div>
                <div className="flex flex-wrap items-center gap-3 mb-3">
                  <CountdownTimer res={res} onExpire={expireReservation} />
                  <span className="font-bold" style={{ fontFamily: "'Playfair Display', serif", fontSize: 16, color: "#4A3728" }}>{fmt(res.book.price)}</span>
                </div>

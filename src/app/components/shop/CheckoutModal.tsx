...
import { validateCouponCode, useBirthdayCoupon } from "../../utils/birthdayEmail";
...
// M2-HU9 + M1-HU7: Stock por tienda
// Estrategia: si el admin ha configurado inventario por tienda en el contexto,
// se usa el valor real. Si no, se hace fallback a la distribución simulada
// basada en book.id (compatibilidad con la Iteración 2).
function simulateStoreStock(book: Book, storeId: number): number {
...
function makeGetStoreStock(
  storeInventory: Record<number, Record<number, number>>
) {
  return (book: Book, storeId: number): number => {
    const real = storeInventory?.[storeId]?.[book.id];
    if (typeof real === "number") return real;
    return simulateStoreStock(book, storeId);
  };
}

function makeCheckStoreAvailability(
  storeInventory: Record<number, Record<number, number>>
) {
  const getStock = makeGetStoreStock(storeInventory);
  return (cartItems: { book: Book; qty: number }[], storeId: number): boolean =>
    cartItems.every(item => getStock(item.book, storeId) >= item.qty);
}

function makeFindNearestAvailableStore(
  storeInventory: Record<number, Record<number, number>>
) {
  const check = makeCheckStoreAvailability(storeInventory);
  return (
    cartItems: { book: Book; qty: number }[],
    excludeStoreId: number
  ): typeof STORES[0] | null => {
    const available = STORES
      .filter(s => s.id !== excludeStoreId && check(cartItems, s.id))
...
export function CheckoutModal({ open, onClose, onSuccess, onGoToWallet }: CheckoutModalProps) {
  const { cart, clearCart, addPurchase, processPurchase, updateBook, showToast, user, storeInventory } = useShop();
  // M1-HU7: closures que usan el inventario actual del contexto
  const getStoreStock = useMemo(() => makeGetStoreStock(storeInventory), [storeInventory]);
  const checkStoreAvailability = useMemo(() => makeCheckStoreAvailability(storeInventory), [storeInventory]);
  const findNearestAvailableStore = useMemo(() => makeFindNearestAvailableStore(storeInventory), [storeInventory]);
  ...
  const [appliedCoupon, setAppliedCoupon]   = useState<BirthdayCoupon | null>(null);
  const [couponError, setCouponError]       = useState("");
  // M2-HU11: ubicación geocodificada de la dirección del usuario (marcador en el mapa)
  const [userLocation, setUserLocation] = useState<{ lat: number; lng: number; label?: string } | null>(null);

  const subtotal      = cart.reduce((s, i) => s + i.book.price * i.qty, 0);
  const shippingCost  = delivery === "shipping" ? 8000 : 0;
  const uniqueBooksInCart = new Set(cart.map(i => i.book.id)).size;
  ...
  }, [cart, selectedStore, selectedStoreHasStock, findNearestAvailableStore]);

  // M2-HU11: Geocodificar la dirección del usuario (Photon/komoot) para mostrar
  // su ubicación en el mapa y calcular la tienda más cercana real.
  useEffect(() => {
    if (delivery !== "pickup") return;
    const addr = address?.trim();
    if (!addr || addr.length < 6) { setUserLocation(null); return; }

    let cancelled = false;
    const t = setTimeout(async () => {
      try {
        const res = await fetch(
          `https://photon.komoot.io/api/?q=${encodeURIComponent(addr)}&lang=es&limit=1`
        );
        const data = await res.json();
        const feat = data?.features?.[0];
        const coords = feat?.geometry?.coordinates; // [lng, lat]
        if (!cancelled && Array.isArray(coords) && coords.length === 2) {
          setUserLocation({ lat: coords[1], lng: coords[0], label: addr });
        }
      } catch {
  ...
  function reset() {
    ...
    setCardCvv(""); setCardCvvError("");
    setCouponCode(""); setAppliedCoupon(null); setCouponError("");
  }

  function handleApplyCoupon() {
    const trimmed = couponCode.trim();
    if (!trimmed) { setCouponError("Ingresa un código de cupón"); return; }
    if (uniqueBooksInCart !== 1) {
      setCouponError("El cupón solo aplica a compras de un único título de libro");
      return;
    }
    const coupon = validateCouponCode(trimmed);
    if (!coupon) { setCouponError("Código inválido, ya fue usado o expiró"); return; }
  ...
  function handleStep2Next() {
    ...
    if (paymentMethod === "wallet") {
      ...
      if (result.success) {
        if (appliedCoupon) useBirthdayCoupon(appliedCoupon.code);
        setOrderId(id);
    ...
    // Validar CVV obligatorio para pago con tarjeta
    if (cardCvv.length < 3) {
      setCardCvvError("El CVV es obligatorio (3–4 dígitos)");
      setProcessing(false);
      return;
    }
    ...
    setTimeout(() => {
      ...
      // Descontar stock del inventario (mismo comportamiento que processPurchase con billetera)
      newPurchase.items.forEach(item => {
        const newStock = Math.max(0, item.book.stock - item.qty);
        updateBook(item.book.id, { stock: newStock, available: newStock > 0 });
      });
      clearCart();
      setProcessing(false);
      setStep(3);
        ...
        <div className="px-8 py-6">
            ...
            <div className="space-y-4">
              ...
              {delivery === "pickup" && (
                <div className="space-y-2 mt-2">
                  ...
                  {/* M2-HU10: Mapa interactivo de tiendas — visible para todo el carrito.
                  ...
                  <div className="mb-3">
                    <StoreMap
                      ...
                      resolveStock={(storeId, book) => getStoreStock(book, storeId)}
                      height={300}
                    />
                  </div>
          ...
          {step === 2 && (
            <div>
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    ...
                    <div>
                      ...
                      <input
                        ...
                        style={{
                          border: `1.5px solid ${cardCvvError ? "#C0392B" : "#E8C99A"}`,
                          background: "#FEFAE0", color: "#4A3728",
                        }}
                      />
                      {cardCvvError && (
                        <p className="text-xs mt-1" style={{ color: "#C0392B" }}>{cardCvvError}</p>
                      )}
                    </div>
                  </div>
                </div>
              ...
              {/* Cupón de cumpleaños */}
              <div className="mb-4 rounded-xl p-4" style={{ background: "#FFF8E7", border: "1.5px dashed #D4A373" }}>
                <p className="text-xs font-bold mb-2" style={{ color: "#4A3728" }}>
                  🎂 ¿Tienes un cupón de cumpleaños?
                  {uniqueBooksInCart !== 1 && (
                    <span className="ml-1 font-normal opacity-60">(solo para compras de 1 título)</span>
                  )}
                </p>
                {appliedCoupon ? (
                  <div className="flex items-center justify-between">
                    <div>
                      <span className="font-mono font-bold text-sm" style={{ color: "#606C38" }}>{appliedCoupon.code}</span>
                      <span className="ml-2 text-xs font-semibold" style={{ color: "#606C38" }}>− {fmt(couponDiscount)} (15%)</span>
                    </div>
                    <button onClick={() => { setAppliedCoupon(null); setCouponCode(""); }}
                      className="text-xs px-2 py-1 rounded-lg" style={{ background: "rgba(192,57,43,0.1)", color: "#C0392B" }}>
                      Quitar
                    </button>
                  </div>
                ) : (
                  <div className="flex gap-2">
                    <input
                      ...
                      placeholder="CUMPLE-XXXXXX"
                      className="flex-1 px-3 py-2 rounded-lg text-sm font-mono outline-none"
                      style={{ border: `1.5px solid ${couponError ? "#C0392B" : "#E8C99A"}`, background: "#FEFAE0", color: "#4A3728" }}
                    />
              ...
              <div className="rounded-xl p-4" style={{ background: "#F5EDD3" }}>
                ...
                {cart.map(i => (
                  <div key={i.book.id} className="flex justify-between text-xs mb-1.5" style={{ color: "#6B5344" }}>
                    <span className="truncate mr-2">
                      <span className="font-mono opacity-60">#{i.book.id}</span> {i.book.title} ×{i.qty}
                    </span>
                ...
                {appliedCoupon && (
                  <div className="flex justify-between text-xs mb-1.5 font-semibold" style={{ color: "#606C38" }}>
                    <span>🎂 Descuento cumpleaños (15%)</span>
          ...
          {step === 2 && (
            <>
              ...
              <button
                ...
                disabled={
                  processing ||
                  (paymentMethod === "wallet" && (user?.balance ?? 0) < total) ||
                  (paymentMethod === "card" && cardCvv.length < 3)
                }

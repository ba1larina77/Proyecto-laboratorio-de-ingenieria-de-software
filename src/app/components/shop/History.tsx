    ...
    >
      ...
      >
            ...
            {purchase.items.map((item, i) => (
              <div key={i} className="flex gap-3 items-center py-2.5 border-b" style={{ borderColor: "#F5EDD3" }}>
                ...
                <div className="flex-1">
                  ...
                  <p className="text-[10px] font-mono mt-0.5" style={{ color: "#6B5344", opacity: 0.6 }}>ID libro: {item.book.id}</p>
                </div>
        ...
        <div className="flex gap-3 px-7 pb-6">
          ...
          {/* Los pedidos en preparación o en camino ya no se pueden cancelar.
              Un pedido en camino solo puede devolverse una vez entregado. */}
          <button
            onClick={onClose}
                    ...
                    <tr key={p.id} className="border-b last:border-0 hover:bg-stone-50 transition-colors"
                      ...
                      <td className="px-5 py-4">
                        {p.items.map((item, i) => (
                          <div key={i} className="text-xs truncate max-w-36" style={{ color: "#6B5344" }}>
                            <span className="font-mono opacity-60">#{item.book.id}</span> {item.book.title} ×{item.qty}
                          </div>
                        ))}
                      </td>
                      ...
                      <td className="px-5 py-4">
                        <div className="flex gap-1.5">
                          ...
                          {/* Pedidos en preparación / en camino ya no son cancelables. */}
                        </div>
                      </td>
                    </tr>
                  ))}

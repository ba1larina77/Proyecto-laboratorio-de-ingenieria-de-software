...
import {
  isTodayBirthday, hasBirthdayEmailBeenSentToday,
  createBirthdayCoupon, sendBirthdayEmail,
  getActiveBirthdayCoupon, validateCouponCode,
} from "../utils/birthdayEmail";
import type { BirthdayCoupon } from "../utils/birthdayEmail";
import { BirthdayBonusModal } from "./BirthdayBonusModal";
...
export function Shop() {
  ...
  // Detección de cumpleaños al cargar con sesión activa
  useEffect(() => {
    if (!user || user.role !== "cliente" || !user.fechaNacimiento) return;
    if (!isTodayBirthday(user.fechaNacimiento)) return;
    if (hasBirthdayEmailBeenSentToday(user.id)) {
      const existing = getActiveBirthdayCoupon(user.id);
      if (existing) { setBirthdayCoupon(existing); setBirthdayModalOpen(true); }
      return;
    }
  ...
  // Manejo del link del email: /?bono=cumpleanos&code=CUMPLE-XXXXX
  useEffect(() => {
    const params = new URLSearchParams(location.search);
    if (params.get("bono") !== "cumpleanos") return;

...
// URL base del backend. En local cae a localhost:3001; en producción se
// define con la variable VITE_API_URL (ej. https://biblion-api.onrender.com).
...
export function ShopProvider({ children }: { children: ReactNode }) {
  ...
  const cancelOrder = useCallback((orderId: string) => {
    ...
    if (!order) return;
    // Regla de negocio: un pedido ya no puede cancelarse una vez está
    // "en preparación" ni "en camino". Los pedidos en camino solo pueden
    // devolverse una vez entregados. Estados entregado/devuelto/cancelado
    // tampoco son cancelables.
    if (order.status === "preparing") {
      showToast("⚠️ No se pueden cancelar pedidos que ya están en preparación", "error");
      return;
    }
    if (order.status === "transit") {
      showToast("⚠️ No se pueden cancelar pedidos en camino. Podrás solicitar la devolución una vez entregado.", "error");
      return;
    }
    showToast("⚠️ Este pedido ya no se puede cancelar", "error");
    return;

    // (Bloque de cancelación deshabilitado: con la regla actual ningún
    //  estado permite cancelar. Se conserva por si se reintroduce un estado
    //  cancelable en el futuro.)
  ...
  const sendBotMessage = useCallback(async (text: string) => {
    ...
    try {
      ...
      const resp = await fetch(`${API_URL}/api/bot/message`, {
        method: 'POST',
